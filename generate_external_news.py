...
2. 社内向けニュースの番外編のような文体で記述（親しみやすく、カジュアルに）
3. ハッシュタグを2-3個含める
4. 絵文字を適度に使用
5. 話題の元になったチャンネル名を明記
...
文体・トーンの重要なルール：
## 口語表現や短縮形
- 「〜ですよね」「〜じゃないですか」などの口語を適度に使う
- 「それって」「だから」などの日常会話表現を入れる
- 堅苦しい表現を避ける

## 比喩や例え話
- 専門的な内容は必ず日常的な比喩で説明
- 例:「これは料理で言えば〜」「まるで○○のように」
- 抽象的な概念を具体的なイメージに変換

## 読者への直接的な語りかけ
- 「あなたも経験ありませんか?」
- 「こう考えてみてください」
- 二人称(あなた)を文章全体で5回以上使う

## 適度な不完全さ
- 完璧すぎる文章を避ける
- 「○○かもしれません」など断定を避ける表現を入れる
- 時には曖昧さを残す
...
"""
            ...
            response = self.openai_client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "あなたは企業のSNS担当者です。社内のSlackメッセージから、社外に共有するのに適切な話題を見つけ、Twitter/X投稿用の親しみやすく人間味のある日本語文章を作成することが得意です。文章は「社内向けニュースの番外編」のような文体で、AIっぽさを感じさせない自然で会話的なトーンを心がけてください。口語表現、比喩、読者への語りかけを積極的に使い、堅苦しさを避けてください。必ず日本語で投稿文を生成してください。"},
